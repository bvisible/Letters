...
class CustomHtmlRenderer(BlockRenderer):
    ...
    def render(self, block: dict[str, Any]) -> str:
        ...
        # //// Neoffice: the email-card <td> zeroes font-size/line-height (the
        # inline-gap killer in the outer wrapper); every other renderer resets
        # them inside its own markup, but raw author HTML inherits them — any
        # multi-line content without an explicit line-height collapsed onto a
        # single overlapping line. Reset here; author inline styles still win.
        wrapped = (
